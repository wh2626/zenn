...
var body: some View {
  ChildView().environment(viewModel)
}
```
...
子ViewにViewModelを渡す構成は取らず、`ParentSettingsScreen`がViewModelを所有する形に統一しました。画面内のUIは`private var`や`private func`に切り出して整理し、ViewとViewModelの1対1を保つようにしています。
